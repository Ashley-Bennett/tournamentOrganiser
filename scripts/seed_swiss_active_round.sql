-- Seed a swiss tournament: 6 players, 2 completed rounds, round 3 active.
-- Run with:
--   psql postgresql://postgres:postgres@localhost:54322/postgres -f scripts/seed_swiss_active_round.sql
...
BEGIN
  ...
  VALUES (v_tourn_id, 'Test Tournament (6p)', v_user_id, 'active', 'swiss', 3, v_ws_id);
  ...
  -- Round 3 (active, no results yet)
  INSERT INTO public.tournament_matches
    (tournament_id, round_number, player1_id, player2_id, status, match_number, pairings_published, workspace_id)
  VALUES
    (v_tourn_id, 3, p1, p5, 'ready', 1, true, v_ws_id),
    (v_tourn_id, 3, p3, p4, 'ready', 2, true, v_ws_id),
    (v_tourn_id, 3, p2, p6, 'ready', 3, true, v_ws_id);

  -- Standings after rounds 1 & 2
  -- p1: 2W, p5: 2W, p3: 1W 1L, p4: 1W 1L, p2: 0W 2L, p6: 0W 2L
...
-- DELETE FROM public.tournaments WHERE name = 'Test Tournament (6p)' AND workspace_id = '79ceaa77-603f-4718-abeb-602946e1d8d8';
