-- Add a persistent organiser broadcast announcement to tournaments.
-- Separate from round_note (which is for round-specific operational notes).

ALTER TABLE public.tournaments ADD COLUMN IF NOT EXISTS announcement TEXT;

-- ── get_player_tournament_view (updated) ────────────────────────────────────
-- Adds announcement to the tournament object returned to players.
...
BEGIN
  ...
  SELECT id, name, status, num_rounds,
         ...
         round_elapsed_seconds, round_is_paused, round_note, announcement
  INTO v_tournament
  ...
  SELECT jsonb_build_object(
    'tournament', jsonb_build_object(
      ...
      'announcement',             v_tournament.announcement
    ),
