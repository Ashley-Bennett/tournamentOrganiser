-- Remove round_note in favour of the announcement broadcast column.

ALTER TABLE public.tournaments DROP COLUMN IF EXISTS round_note;

-- ── get_player_tournament_view (updated) ────────────────────────────────────
-- Removes round_note from the tournament object.
...
BEGIN
  ...
  SELECT id, name, status, num_rounds,
         ...
         round_elapsed_seconds, round_is_paused, announcement
  INTO v_tournament
