-- ── player_agreement_stays_pending ───────────────────────────────────────────
-- When both players agree on a result the match was previously auto-completed.
-- Only the organiser should be able to set status='completed', so we remove the
-- auto-complete from the player_agreement path.
--
-- The match now stays pending with confirmed_by='player_agreement' and
-- temp_winner_id / temp_result pre-filled (same as player_report / conflict),
-- so the organiser's "pending results" restore logic picks it up automatically.
