-- ============================================================
-- Game-neutral join codes (2026-09-01) — 1.0 Phase D
--
-- Room codes were drawn from a Pokémon name list for every
-- tournament, so a generic event handed players codes like
-- GOLDUCK. Codes now follow the tournament's game:
--   pokemon  -> the existing Pokémon list, unchanged
--   anything -> a neutral list of short, memorable words
--
-- generate_join_code() gains a game argument. The BEFORE INSERT
-- trigger is where every code is actually born, and it has
-- NEW.game_id in scope, so that is where the game is passed.
--
-- set_tournament_join_enabled() and set_tournament_late_join()
-- also call generate_join_code(), but only on the branch where an
-- existing tournament has a NULL join_code. The insert trigger has
-- assigned one to every tournament since 20260330000002 and nothing
-- ever clears it, so that branch is unreachable; those functions are
-- deliberately left untouched rather than re-declared (they carry
-- the workspace role guards from 20260825130000) and fall through to
-- the neutral list if they ever do fire.
-- ============================================================

DROP FUNCTION IF EXISTS public.generate_join_code();

CREATE FUNCTION public.generate_join_code(p_game_id TEXT DEFAULT NULL)
RETURNS TEXT
LANGUAGE plpgsql
SECURITY DEFINER
...
-- ---- Pass the game through at the point codes are created ----
