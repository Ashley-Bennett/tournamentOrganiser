-- Fix: "column reference tournament_id is ambiguous" in get_my_player_entries.
-- PostgreSQL's correlated subquery resolution brought outer tp.tournament_id into
-- scope alongside my_tournament_ids.tournament_id. Replaced IN (SELECT ...) with
-- an explicit JOIN to eliminate the ambiguity.
